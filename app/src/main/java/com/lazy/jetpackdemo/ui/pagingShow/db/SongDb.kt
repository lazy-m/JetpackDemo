...
import com.lazy.jetpackdemo.utilities.ioThread
...
abstract fun songDao():SongDao
    companion object{
        ...
        fun  get(context:Context): SongDb {
            if (instance==null){
                instance= Room.databaseBuilder(context.applicationContext,
                    SongDb::class.java,"Song").addCallback(object :RoomDatabase.Callback(){
       ...
       private fun fillInDb(context: Context){
           ioThread {
               get(context).songDao().insert(CHEESE_DATA.map { Song(id = 0, songName = it) })
           }
        }
    }
}
